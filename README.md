...
| `com.coffeebean.core` | 模块管理 / 引导 / 服务注册（Window/CoffeeBean 工具中心：模块管理 + 构建模式 Beta/Release 切换，含 COFFEEBEAN_CORE 宏安装） | `https://github.com/Herschy0829/com.coffeebean.core.git#v0.1.71` |
...
| `com.coffeebean.excel` | Excel 配置表工具链（Editor-only：多 Sheet/分章节/增量批量/生成到内嵌包且一表一文件夹（Code/ + Data/）/数据为 .cbcfg 容器（Deflate 压缩 + XOR 加密，构建时由 BuildPlayerProcessor 挂进 StreamingAssets）/生成代码独立 asmdef + CoffeeBean 统一命名空间） | `https://github.com/Herschy0829/com.coffeebean.excel.git#v0.6.0` |
...
| `com.coffeebean.tools` | 工具模块（单例 / 主线程调度 / 线程池 / 构建模式门面 CGameBuild，CLog 按 Beta/Release 剥离；**固定硬依赖 UniRx + UniTask**，由 Core registry 的 externalDependencies 一起装） | `https://github.com/Herschy0829/com.coffeebean.tools.git#v0.13.0` |
