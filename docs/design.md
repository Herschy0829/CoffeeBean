...
> 状态：**已实施** —— 但请先读下面的「架构演进」，本文档的部分原始原则已被实际架构取代

---

## 0. 架构演进（阅读本文档前必读）

本文档是最初的 v0.2 设计定稿，**框架已按它建成并持续演进**。有两处原始决策在实施过程中被推翻，
直接读下文会产生误解，故在此显式更正：

| 原始设计（下文仍可见） | 实际落地架构 | 为什么改 |
|---|---|---|
| **原则 4**：所有模块 → 依赖 Core（Core 是唯一无依赖的模块） | **Core 可选**：模块主程序集零 Core 依赖，只在 `Runtime/Bridge/` 里通过条件编译接入 Core | 让模块能脱离 Core 独立使用；也避免"只想用对象池却被迫拖进整个框架" |
| **原则 2**：模块之间**禁止横向依赖** | **允许单向依赖更底层的模块**（如 `net→tools`、`ui→tools+asset`、`ad→tools+telemetry`、`purchase→excel`） | 纯粹"零横向依赖"会把通用能力重复实现；改为依赖**职责更底层**的模块，横向业务需求仍走服务注册表 |

**Core 可选（Bridge 模式）的机制**（14/16 个模块采用，模板已同步）：

1. 模块主 Runtime asmdef 用 `versionDefines` 声明：工程里存在 `com.coffeebean.core` → 定义 `COFFEEBEAN_CORE`
2. `Runtime/Bridge/` 下的独立 asmdef 带 `defineConstraints: ["COFFEEBEAN_CORE"]`，**只在装了 Core 时才编译**
3. 模块标记 `[assembly: CoffeeBeanModule]` 与 `ICoffeeBeanModule` 生命周期就写在 Bridge 里
4. Core 的 `CoffeeBeanDefineInstaller` 在编辑器初始化时自动注入全局宏 `COFFEEBEAN_CORE`

因此：**不装 Core，模块照常独立工作；装了 Core，自动被框架发现、启停与版本校验。**
...
| 2 | 单一职责 | 一个模块只做一件事；模块间**允许单向依赖职责更底层的模块**（见 §0），横向业务需求走服务注册表 |
| 3 | Core 最小化 | Core 只负责"管理"（注册/引导/服务定位/装卸载工具），业务能力全部下沉到各模块 |
| 4 | 依赖单向且 Core 可选 | 模块可依赖更底层模块；**对 Core 的依赖是可选的条件编译集成**（Bridge 模式，见 §0），横向需求通过服务注册表解耦 |
